...
const clientId = "d789be9fca274221a58ae8b6337d50f0";
...
const Spotify = {
    getAccessToken() {
        ...
        if (expiresInMatch && accessTokenMatch) {
            accessToken = accessTokenMatch[1];
            const expiresIn = Number(expiresInMatch[1]);
            window.setTimeout(() => (accessToken = ""), expiresIn * 1000);
            window.history.pushState("Access Token", null, "/");//this clear paramter   
            return accessToken;
        } else {
    ...
    savePlaylist(name, trackUris) {
        ...
        return fetch("http://api.spotify.com/v1/me", { headers: headers })
            .then(response => response.json())
            .then(jsonResponse => {
                userId = jsonResponse.id;
                return fetch(`http://api.spotify.com/v1/user/${userId}/playlists`, {
                    headers: headers,
                    method: 'POST',
